...
int main()
{
	...
	Texture texAlbedo(GL_TEXTURE_2D, "res/objects/rusted_iron/albedo.png");
	Texture texMetallic(GL_TEXTURE_2D, "res/objects/rusted_iron/metallic.png");
	Texture texRoughness(GL_TEXTURE_2D, "res/objects/rusted_iron/roughness.png");
	Texture texNormal(GL_TEXTURE_2D, "res/objects/rusted_iron/normal.png");

	std::vector<std::shared_ptr<Texture>> textures = std::vector<std::shared_ptr<Texture>>();
	textures.push_back(std::make_shared<Texture>(texAlbedo));
	textures.push_back(std::make_shared<Texture>(texMetallic));
	textures.push_back(std::make_shared<Texture>(texRoughness));
	int i = 0;
	for (auto iter = textures.begin(); iter != textures.end(); ++iter, ++i)
	{
		(*iter)->setSamplerName("tex_diffuse", i);
	}

	texNormal.setSamplerName("tex_normal", 0);
	textures.push_back(std::make_shared<Texture>(texNormal));

	SphereMesh sphere(textures);
	//Model backpack("res/objects/backpack/backpack.obj");

	Shader pbrShader(SHADER_PATH("01_PBR_lighting_withTexture.vs"), SHADER_PATH("01_PBR_lighting_withTexture.fs"));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		{
			pbrShader.setFloat("metallicProperty", (float)row / (float)sphereRow);
			for (int col = 0; col < sphereColumn; ++col)
			{
				...
				pbrShader.setFloat("roughnessProperty", glm::clamp((float)col / (float)sphereColumn, 0.1f, 1.0f));
