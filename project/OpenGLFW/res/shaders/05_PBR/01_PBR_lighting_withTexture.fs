...
uniform sampler2D tex_diffuse1; // metallic
uniform sampler2D tex_diffuse2; // roughness
uniform sampler2D tex_normal0;

// material parameters
// uniform vec3 albedo;
uniform float metallicProperty;
uniform float roughnessProperty;
uniform float ao;
...
// ----------------------------------------------------------------------------
mat3 genTangentToWorldTBN(vec3 worldPos, vec2 uv, vec3 normalInWorld) {
...
{
    vec3 tangentNormal = texture(tex_normal0, fs_in.texCoords).xyz * 2.0 - 1.0;
    mat3 TBN = genTangentToWorldTBN(fs_in.worldPos, fs_in.texCoords, fs_in.normal);
...
void main()
{
	vec3 albedo = pow(texture(tex_diffuse0, fs_in.texCoords).rgb, vec3(2.2));
	float metallic = texture(tex_diffuse1, fs_in.texCoords).r * metallicProperty;
	float roughness = texture(tex_diffuse2, fs_in.texCoords).r * roughnessProperty;
